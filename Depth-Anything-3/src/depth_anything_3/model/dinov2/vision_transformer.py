...
class DinoVisionTransformer(nn.Module):
    ...
    def _build_token_validity(self, masks, B, S, H, W, device):
        if masks is None:
            return None
        if masks.dim() == 5 and masks.shape[2] == 1:
            masks = masks[:, :, 0]
        if masks.dim() != 4:
            raise ValueError(f"Expected masks as (B,S,H,W), got {tuple(masks.shape)}")
        if masks.shape[:2] != (B, S):
            raise ValueError(
                f"Expected masks batch/view dims {(B, S)}, got {tuple(masks.shape[:2])}"
            )

        patch_h = patch_w = int(self.patch_size)
        patch_masks = masks.to(device=device, dtype=torch.float32).reshape(B * S, 1, H, W)
        patch_masks = nn.functional.max_pool2d(
            patch_masks,
            kernel_size=(patch_h, patch_w),
            stride=(patch_h, patch_w),
        ).reshape(B, S, -1) > 0

        view_valid = patch_masks.any(dim=-1, keepdim=True)
        special_valid = view_valid.expand(-1, -1, 1 + self.num_register_tokens)
        return torch.cat((special_valid, patch_masks), dim=-1)

    def _build_attention_mask(self, token_valid, attn_type):
        if token_valid is None:
            return None
        if attn_type == "local":
            return rearrange(token_valid, "b s n -> (b s) 1 n")
        if attn_type == "global":
            return rearrange(token_valid, "b s n -> b 1 (s n)")
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        token_valid = self._build_token_validity(kwargs.get("masks"), B, S, H, W, x.device)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.alt_start != -1 and (i == self.alt_start - 1) and x.shape[1] >= THRESH_FOR_REF_SELECTION and kwargs.get("cam_token", None) is None:
                ...
                if token_valid is not None:
                    token_valid = reorder_by_reference(token_valid, b_idx)
            ...
            if self.alt_start != -1 and i >= self.alt_start and i % 2 == 1:
                x = self.process_attention(
                    x, blk, "global", pos=g_pos, token_valid=token_valid
                )
            else:
                x = self.process_attention(x, blk, "local", pos=l_pos, token_valid=token_valid)
                local_x = x
    ...
    def process_attention(self, x, block, attn_type="global", pos=None, token_valid=None):
        b, s, n = x.shape[:3]
        attn_mask = self._build_attention_mask(token_valid, attn_type)
        if attn_type == "local":
            x = rearrange(x, "b s n c -> (b s) n c")
            if pos is not None:
                pos = rearrange(pos, "b s n c -> (b s) n c")
            if token_valid is not None:
                token_valid = rearrange(token_valid, "b s n -> (b s) n")
